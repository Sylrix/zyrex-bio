...
const SHARED_SPECS = [
  ...
  { label: 'Storage', value: 'Store sealed at 2–8°C, protected from light' },
  { label: 'Intended use', value: 'Laboratory research use only' },
  { label: 'Origin', value: 'Dispatched from Melbourne, Australia' },
] as const

function packDescription(vials: number): readonly string[] {
  const unit = vials === 1 ? 'vial' : 'vials'
  return [
    `${vials} × 500mg NAD+ ${unit} supplied as a lyophilised powder in sealed glass vials, tested at 99% purity or better.`,
    'Each vial is sealed with a butyl stopper and aluminium flip-off cap, labelled with the compound, quantity and batch, and packed in a moulded insert so nothing moves in transit.',
